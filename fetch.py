...
def get_daily_forecast(lat, lon, name):
    ...
    return (
        name, tomorrow,
        data["temperature_2m_max"][0], data["precipitation_sum"][0],
        data["wind_speed_10m_max"][0], int(data["cloud_cover_mean"][0]),
...
print(f"✅ Fetched data for {len(weather_records)} locations.")
...
conn.commit()
conn.close()
print(f"💾 Saved successfully to {SQL_DB_PATH}.")
